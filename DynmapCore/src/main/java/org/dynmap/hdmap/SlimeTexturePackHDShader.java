...
public class SlimeTexturePackHDShader implements HDShader {
    ...
    public SlimeTexturePackHDShader(DynmapCore core, ConfigurationNode configuration) {
    ...
    class ShaderState implements HDShaderState {
        ...
        public HDShader getShader() {
            return SlimeTexturePackHDShader.this;
        }
        ...
        public boolean processBlock(HDPerspectiveState ps) {
            ...
            if (c.getAlpha() > 0) {
                ...
                double greenTint = 0.25;
                /* If no previous color contribution, use new color */
                if(color[0].isTransparent()) {
                    for(int i = 0; i < color.length; i++)
                        color[i].setColor(tintColor(tmpcolor[i].getRed(), tmpcolor[i].getGreen(),
                                tmpcolor[i].getBlue(), tmpcolor[i].getAlpha(), isSlimeChunk));
                ...
                /* Else, blend and generate new alpha */
                    ...
                    if(talpha > 0) {
                        for (int i = 0; i < color.length; i++) {
                            red = (tmpcolor[i].getRed() * alpha2 + color[i].getRed() * alpha) / talpha;
                            green = (tmpcolor[i].getGreen() * alpha2 + color[i].getGreen() * alpha) / talpha;
                            blue = (tmpcolor[i].getBlue() * alpha2 + color[i].getBlue() * alpha) / talpha;
                            color[i].setColor(tintColor(red, green, blue, talpha, isSlimeChunk));
                        }
                    }
                    else {
                        for (int i = 0; i < color.length; i++)
        ...
        public Color tintColor(int r, int g, int b, int a, boolean isSlime) {
            int tmp = (int)(0.299 * r + 0.587 * g + 0.114 * b);
            double tintValue = 0.4;
            int newRB = isSlime ? (int)(tmp + (0 - tmp) * tintValue): tmp;
            int newG = isSlime ? (int)(tmp + (255 - tmp) * tintValue) : tmp;
            return new Color(newRB, newG, newRB, a);
        }
        /**
